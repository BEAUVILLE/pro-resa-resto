(function(){
  ...
  function getSessionSlug(){
    const resaSession = safeJsonParse(storageGet("DIGIY_RESA_PRO_SESSION"));
    if(resaSession?.slug) return normalizeSlug(resaSession.slug);

    const generic = safeJsonParse(storageGet("DIGIY_PRO_SESSION"));
    if(generic?.module === MODULE && generic?.slug) return normalizeSlug(generic.slug);

    const guardState = window.DIGIY_GUARD?.state || null;
    if(guardState?.slug) return normalizeSlug(guardState.slug);
  ...
  function resolveSlug(){
    return normalizeSlug(
      queryValue("slug") ||
      storageGet(STORAGE_MODULE_SLUG) ||
      storageGet(STORAGE_LAST_SLUG) ||
      storageGet(STORAGE_GENERIC_SLUG) ||
      storageGet(STORAGE_DIGIY_SLUG) ||
      getSessionSlug() ||
      ""
    );
  }
  ...
  function persistSlug(slug){
    ...
    if(!clean) return "";
    storageSet(STORAGE_LAST_SLUG, clean);
    storageSet(STORAGE_MODULE_SLUG, clean);
    storageSet(STORAGE_GENERIC_SLUG, clean);
    storageSet(STORAGE_DIGIY_SLUG, clean);
    return clean;
  }
  ...
  function selectedBookingCard(){
    ...
    const focused = document.querySelector(".booking:focus-within");
  ...
  function readBookingContext(){
    ...
    const name =
      card.getAttribute("data-customer-name") ||
      card.querySelector(".booking-name")?.textContent ||
      "";

    const phone =
      card.getAttribute("data-customer-phone") ||
      card.querySelector("[data-phone]")?.getAttribute("data-phone") ||
      "";
    ...
    const service =
      ...
      card.querySelector(".service-pill")?.textContent ||
      "";
    ...
    const when =
      card.getAttribute("data-booking-date") ||
    ...
    const clean = {
      booking_id: shortText(id, 80),
      customer_name: shortText(name, 80),
      customer_phone: shortText(phone, 40),
      status: shortText(status, 40),
      service_name: shortText(service, 80),
      when: shortText(when, 160),
      note: shortText(note, 200)
    };
  ...
  function buildPrompt(kind = "general"){
    ...
    lines.push(`SLUG: ${ctx.slug || "non détecté"}`);
    ...
    if(kind === "booking_diagnostic"){
      ...
      lines.push(`4. la prochaine étape interne dans RESA.`);
    }else if(kind === "day_brief"){
      lines.push(`Donne-moi un brief terrain pour ma journée RESA.`);
      lines.push(`Je veux:`);
      ...
      lines.push(`Je veux 3 versions:`);
      lines.push(`- confirmation,`);
      lines.push(`- rappel,`);
      lines.push(`- report / ajustement.`);
      ...
      lines.push(`Aide-moi à piloter mon module RESA depuis cette page.`);
  ...
  function buildFloatingStyles(){
    return `
      ...
      .digiy-claw-btn:hover{
        transform:translateY(-1px);
      }

      .digiy-claw-btn.secondary{
        background:rgba(255,255,255,.04);
      }
    ...
    return `
      <div class="digiy-claw-head">
        <div class="digiy-claw-title">
          <div class="digiy-claw-eyebrow">CLAW by DIGIY</div>
          <h3>Aide intelligente RESA</h3>
          <p>Lecture terrain, messages client, diagnostic réservation, et circulation interne propre.</p>
        </div>
      ...
      <div class="digiy-claw-body">
        <section class="digiy-claw-box">
          ...
          <div class="digiy-claw-meta">
            ...
            <span class="digiy-claw-pill">Slug : ${esc(ctx.slug || "—")}</span>
          </div>
          <div class="digiy-claw-meta">
        ...
        <section class="digiy-claw-box">
          ...
          <div class="digiy-claw-actions">
            ...
            <button class="digiy-claw-btn ${selected ? "" : "secondary"}" type="button" data-claw-action="booking_diagnostic">
              Préparer un diagnostic réservation
            </button>
          </div>
        ...
        <section class="digiy-claw-box">
          <h4>Navigation interne</h4>
          <div class="digiy-claw-links">
            <a class="digiy-claw-link" href="${esc(ctx.links.cockpit)}">Cockpit</a>
            <a class="digiy-claw-link" href="${esc(ctx.links.dashboard)}">Dashboard</a>
            <a class="digiy-claw-link" href="${esc(ctx.links.planning)}">Planning</a>
            <a class="digiy-claw-link" href="${esc(ctx.links.profile)}">Profil</a>
            <a class="digiy-claw-link" href="${esc(ctx.links.qr)}">QR</a>
            <a class="digiy-claw-link" href="${esc(ctx.links.claw)}">Page CLAW</a>
          </div>
        </section>
      </div>
    `;
  ...
  function createFab(){
    ...
    btn.id = "digiyClawFab";
    btn.innerHTML = `<span>🦅</span><span>Ouvrir CLAW</span>`;
    return btn;
  }
  ...
  function createPanel(){
    ...
    panel.setAttribute("aria-label", "CLAW by DIGIY");
  ...
  function bindPanelEvents(panel){
    ...
    panel.querySelectorAll("[data-claw-action]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if(kind === "day_brief"){
          setStatus("Brief de journée préparé.");
        }else if(kind === "client_message"){
          setStatus("Messages client préparés.");
        }else if(kind === "profile_improve"){
          setStatus("Analyse profil préparée.");
        }else if(kind === "booking_diagnostic"){
          setStatus("Diagnostic réservation préparé.");
        }else{
          setStatus("Prompt général préparé.");
        }
      });
    });
  }
  ...
  function mount(options = {}){
    ...
    if(options.internalOnly !== false && !isInternalPage()){
    ...
    if(storageGet(STORAGE_PANEL_STATE) === "1"){
      setPanelOpen(true);
    }
  ...
  function open(){
    if(!document.getElementById("digiyClawFab")){
      mount();
    }
    refreshPanel();
    setPanelOpen(true);
